...
public class MultiSceneNetManager : NetworkManager
{
    ...
    {
        Debug.Log("MultiSceneNetManager Awake");
        base.Awake();
        singleton = this;
    }
    ...
    {
        Debug.Log("MultiSceneNetManager OnServerAddPlayer");
    ...
    IEnumerator OnServerAddPlayerDelayed(NetworkConnectionToClient conn)
    {
        ...
        Debug.Log("MultiSceneNetManager OnServerAddPlayerDelayed conn.Send new SceneMessage");
        conn.Send(new SceneMessage { sceneName = gameScene, sceneOperation = SceneOperation.LoadAdditive });

        // Wait for end of frame before adding the player to ensure Scene Message goes first
        Debug.Log("MultiSceneNetManager OnServerAddPlayerDelayed new WaitForEndOfFram start");
        yield return new WaitForEndOfFrame();
        Debug.Log("MultiSceneNetManager OnServerAddPlayerDelayed new WaitForEndOfFram end");

        base.OnServerAddPlayer(conn);

        PlayerScore playerScore = conn.identity.GetComponent<PlayerScore>();
        playerScore.playerNumber = clientIndex;
        Debug.Log("MultiSceneNetManager set playerNumber"+ clientIndex);
        playerScore.scoreIndex = clientIndex / subScenes.Count;
        playerScore.matchIndex = clientIndex % subScenes.Count;
        Debug.Log("MultiSceneNetManager set scoreIndex" + clientIndex / subScenes.Count);
        Debug.Log("MultiSceneNetManager set matchIndex" + clientIndex % subScenes.Count);
        ...
        {
            Debug.Log("MultiSceneNetManager MoveGameObjectToScene" + subScenes[clientIndex % subScenes.Count].name);
    ...
    {
        Debug.Log("MultiSceneNetManager OnStartServer");
    ...
    IEnumerator ServerLoadSubScenes()
    {
        for (int index = 1; index <= instances; index++)
        {
            ...
            Debug.Log(index+ "| MultiSceneNetManager ServerLoadSubScenes");
            Scene newScene = SceneManager.GetSceneAt(index);
            subScenes.Add(newScene);
            Debug.Log(index + "| MultiSceneNetManager ServerLoadSubScenes"+newScene.name);
    ...
    {
        Debug.Log("MultiSceneNetManager OnStopServer");
    ...
    IEnumerator ServerUnloadSubScenes()
    {
        ...
        {
            Debug.Log(index+"| MultiSceneNetManager ServerUnloadSubScenes"+ subScenes[index].name);
    ...
    {
        Debug.Log("MultiSceneNetManager OnStopClient");
    ...
    IEnumerator ClientUnloadSubScenes()
    {
        ...
        {
            Debug.Log(index + "| MultiSceneNetManager ClientUnloadSubScenes" + SceneManager.GetSceneAt(index).name);
